...
class SchedulerService:
    ...
    def _get_symbols_needing_update(
        self, symbols: list[str], asset_type: str, cache_hours: int = 1
    ) -> list[str]:
        """
        ...
        cutoff_time = datetime.now(UTC) - timedelta(hours=cache_hours)

        try:
            # Find symbols with recent tips
            recent_tips = (
                self.db_session.query(TipRecord)
                .filter(
            ...
            structured_logger.warning(
                f"Error checking cache, fetching all symbols: {e!s}",
                context={"asset_type": asset_type},
            )
            return symbols
    ...
    def execute_delivery(self, delivery_type: str) -> None:
        ...
        try:
            ...
            crypto_data = (
                self.market_aggregator.fetch_crypto_data(crypto_to_fetch) if crypto_to_fetch else []
            )
            stock_data = (
                self.market_aggregator.fetch_stock_data(stock_to_fetch) if stock_to_fetch else []
            )
            ...
            if all_success:
                ...
                if self.event_store:
                    self.event_store.add_event(
                        ...
                        context={"delivery_type": delivery_type, "status": "failed"},
                        duration_ms=duration_ms,
                    )
            ...
            duration_ms = (time.time() - start_time) * 1000
            structured_logger.error(
                f"Error during {delivery_type} delivery: {e!s}",
                context={
                    "trace_id": trace_id,
                    "delivery_type": delivery_type,
                    "duration_ms": duration_ms,
    ...
    def _store_tips(self, tips: list, delivery_type: str) -> None:
        ...
        try:
            ...
            structured_logger.error(f"Error storing tips in database: {e!s}", exception=e)
    ...
    def _store_market_data(self, market_data: list) -> None:
        ...
        try:
            ...
            structured_logger.error(f"Error storing market data in database: {e!s}", exception=e)
            self.db_session.rollback()
