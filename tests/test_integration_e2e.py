...
from src.database.models import TipRecord, MarketDataRecord, DeliveryLog
...
class TestSchedulerToEmailFlow:
    ...
    def test_complete_evening_delivery_flow(self, test_session: Session):
        ...
        stock_data = MarketData(
            ...
            source=DataSource(name="Alpha Vantage", url="https://alphavantage.co", fetched_at=datetime.now())
        )
        ...
        with patch.object(scheduler.market_aggregator, 'fetch_crypto_data', return_value=[crypto_data]):
            with patch.object(scheduler.market_aggregator, 'fetch_stock_data', return_value=[stock_data]):
                with patch.object(scheduler.analysis_engine, 'analyze_crypto', return_value=[crypto_tip]):
                    with patch.object(scheduler.analysis_engine, 'analyze_stocks', return_value=[stock_tip]):
                        with patch.object(scheduler.email_service, 'send_email_content', return_value=True) as mock_send:
...
class TestSchedulerToDashboardFlow:
    ...
    def test_market_data_persisted_to_database(self, test_session: Session, test_client: TestClient):
...
class TestErrorScenarios:
    ...
    def test_api_failure_during_data_fetch(self, test_session: Session):
        ...
        # Mock API failure
        with patch.object(scheduler.market_aggregator, 'fetch_crypto_data', side_effect=Exception("API connection failed")):
            with patch.object(scheduler.market_aggregator, 'fetch_stock_data', return_value=[]):
            ...
            with patch.object(scheduler.market_aggregator, 'fetch_stock_data', return_value=[]):
                with patch.object(scheduler.analysis_engine, 'analyze_crypto', return_value=[mock_tip]):
                    with patch.object(scheduler.analysis_engine, 'analyze_stocks', return_value=[]):
                        ...
                        with patch.object(scheduler.email_service, 'send_email_content', return_value=False) as mock_send:
                            scheduler.execute_delivery("morning")
...
class TestEndToEndDashboardFlow:
    ...
    def test_dashboard_tip_history_with_date_range(self, test_session: Session, test_client: TestClient):
